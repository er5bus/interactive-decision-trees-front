...
class ViewContentNode extends React.Component {
  ...
  checkLogicNode = (item) => {
    ...
    if (item.rules){
      item.rules.forEach((rule) => {
        if ( this.checkOperatorLogicNode(rule.value, this.state.scores[rule.score.id], rule.operator )){
          this.props.history.push(ROUTES.USER.MAIN_PATH + ROUTES.USER.TREE_VIEW_CONTENT_NODE.replace(":treeparam", treeparam).replace(":nodeparam", rule.point_to.uid))
          noRuleMatch = false
          return;
        }
      })
    ...
    if (noRuleMatch){
      this.props.history.push(ROUTES.USER.MAIN_PATH + ROUTES.USER.TREE_VIEW_CONTENT_NODE.replace(":treeparam", treeparam).replace(":nodeparam", item.default_node.uid))
    }
  }
                  ...
                  ? <>
                    ...
                    <div>
                      ...
                      { item.actions.map((action, i) =>
                        (item.display_style === "BUTTON" ? <Button
                          key={ action.point_to.uid + i}
                          className="btn mt-4 mr-2"
                          color="primary"
                          disabled={ !Boolean(action.point_to.uid) }
                          to={ ROUTES.USER.MAIN_PATH + ROUTES.USER.TREE_VIEW_CONTENT_NODE.replace(":treeparam", treeparam).replace(":nodeparam", action.point_to.uid) }
                          tag={Link}
                        >
                          { action.name }
                        </Button>
